--// OLEMAD ADMIN PANEL v5.1 - MOBILE OPTIMIZED
--// Toggle: RightShift  |  Drag: header  |  Resize: زر [+] / [-]

if game:GetService("CoreGui"):FindFirstChild("OlemadAdmin") then game:GetService("CoreGui").OlemadAdmin:Destroy() end
if _G.OlemadAdminConns then for _,c in pairs(_G.OlemadAdminConns) do pcall(function() c:Disconnect() end) end end
if _G.OlemadESP then for _,o in pairs(_G.OlemadESP) do for _,d in pairs(o) do pcall(function() d:Remove() end) end end end
_G.OlemadAdminConns={} _G.OlemadESP={}
...
local State={FlySpeed=50,WalkSpeed=16,JumpPower=50,Flying=false,Noclip=false,InfJump=false,Aimbot=false,AimHold=true,AimFov=120,AimSmooth=5,AimPart="Head",AimTeamCheck=false,AimAliveCheck=true,ESP=false,ESPBoxes=true,ESPNames=true,ESPTracers=false,ESPHealth=true,ESPTeam=false,AntiFling=false,Fling=false,PanelScale=1}
...
local function conn(sig,fn) local c=sig:Connect(fn) table.insert(_G.OlemadAdminConns,c) return c end
...
local C={bg=Color3.fromRGB(17,17,20),header=Color3.fromRGB(23,23,27),colbg=Color3.fromRGB(24,24,29),border=Color3.fromRGB(44,44,51),hover=Color3.fromRGB(34,34,40),text=Color3.fromRGB(233,233,238),dim=Color3.fromRGB(150,150,160),faint=Color3.fromRGB(105,105,116),blue=Color3.fromRGB(72,130,248),red=Color3.fromRGB(220,80,90),trackOff=Color3.fromRGB(58,58,66),pill=Color3.fromRGB(33,33,40),pillBrd=Color3.fromRGB(54,54,62),knob=Color3.fromRGB(240,240,245)}
local FONT=Enum.Font.Gotham local FONTM=Enum.Font.GothamMedium local FONTB=Enum.Font.GothamBold
local COLW=152 -- أصغر بكثير للجوال
local gui=Instance.new("ScreenGui") gui.Name="OlemadAdmin" gui.ResetOnSpawn=false gui.ZIndexBehavior=Enum.ZIndexBehavior.Sibling gui.IgnoreGuiInset=true gui.Parent=game:GetService("CoreGui")
...
local WIN_W=COLW*4+10*3+12
local main=Instance.new("Frame") main.Size=UDim2.fromOffset(WIN_W,0) main.AutomaticSize=Enum.AutomaticSize.Y main.Position=UDim2.new(0.5,-WIN_W/2,0,40) main.BackgroundColor3=C.bg main.BorderSizePixel=0 main.Active=true main.Draggable=true main.ClipsDescendants=false main.Parent=gui corner(main,10) strokeOf(main,C.border,0)
...
local header=Instance.new("Frame") header.Size=UDim2.new(1,0,0,34) header.BackgroundColor3=C.header header.BorderSizePixel=0 header.Parent=main corner(header,10)
local hfix=Instance.new("Frame") hfix.Size=UDim2.new(1,0,0,10) hfix.Position=UDim2.new(0,0,1,-10) hfix.BackgroundColor3=C.header hfix.BorderSizePixel=0 hfix.Parent=header
...
local hIcon=lucide(header,"zap",13,C.blue) hIcon.Position=UDim2.fromOffset(10,10)
local tlabel=Instance.new("TextLabel") tlabel.Size=UDim2.new(0.4,0,1,0) tlabel.Position=UDim2.fromOffset(28,0) tlabel.BackgroundTransparency=1 tlabel.Font=FONTB tlabel.Text="Olemad" tlabel.TextColor3=C.text tlabel.TextSize=12 tlabel.TextXAlignment=Enum.TextXAlignment.Left tlabel.Parent=header
local function hdrBtn(txt,xoff,size) local b=Instance.new("TextButton") b.Size=UDim2.fromOffset(size or 22,size or 22) b.Position=UDim2.new(1,xoff,0.5,-(size or 22)/2) b.BackgroundColor3=C.hover b.BackgroundTransparency=1 b.Text=txt b.Font=FONTB b.TextSize=size and size*0.6 or 13 b.TextColor3=C.dim b.AutoButtonColor=false b.Parent=header corner(b,5) b.MouseEnter:Connect(function() b.BackgroundTransparency=0 b.TextColor3=C.text end) b.MouseLeave:Connect(function() b.BackgroundTransparency=1 b.TextColor3=C.dim end) return b end
local closeBtn=hdrBtn("✕",-28,20) local minBtn=hdrBtn("—",-50,20)
-- زر تصغير/تكبير جديد
local sizeBtn=hdrBtn("□",-72,20)
local body=Instance.new("Frame") body.Size=UDim2.new(1,0,0,0) body.AutomaticSize=Enum.AutomaticSize.Y body.Position=UDim2.fromOffset(0,34) body.BackgroundTransparency=1 body.Parent=main
local bpad=Instance.new("UIPadding") bpad.PaddingTop=UDim.new(0,6) bpad.PaddingBottom=UDim.new(0,6) bpad.PaddingLeft=UDim.new(0,6) bpad.PaddingRight=UDim.new(0,6) bpad.Parent=body
local rowL=Instance.new("UIListLayout") rowL.FillDirection=Enum.FillDirection.Horizontal rowL.Padding=UDim.new(0,5) rowL.SortOrder=Enum.SortOrder.LayoutOrder rowL.VerticalAlignment=Enum.VerticalAlignment.Top rowL.Parent=body
local function column(order) local col=Instance.new("Frame") col.Size=UDim2.fromOffset(COLW,0) col.AutomaticSize=Enum.AutomaticSize.Y col.BackgroundColor3=C.colbg col.BorderSizePixel=0 col.LayoutOrder=order col.Parent=body corner(col,6) strokeOf(col,C.border,0.2) local p=Instance.new("UIPadding") p.PaddingTop=UDim.new(0,4) p.PaddingBottom=UDim.new(0,6) p.PaddingLeft=UDim.new(0,4) p.PaddingRight=UDim.new(0,4) p.Parent=col local l=Instance.new("UIListLayout") l.Padding=UDim.new(0,2) l.SortOrder=Enum.SortOrder.LayoutOrder l.Parent=col return col end
local function section(parent,name,iconName,first) local w=Instance.new("Frame") w.Size=UDim2.new(1,0,0,first and 14 or 18) w.BackgroundTransparency=1 w.Parent=parent local xo=2 if iconName then local ic=lucide(w,iconName,10,C.faint) ic.Position=UDim2.new(0,2,1,-10) xo=16 end local l=Instance.new("TextLabel") l.Size=UDim2.new(1,-xo,0,10) l.Position=UDim2.new(0,xo,1,-10) l.BackgroundTransparency=1 l.Font=FONTB l.Text=string.upper(name) l.TextColor3=C.faint l.TextSize=8 l.TextXAlignment=Enum.TextXAlignment.Left l.Parent=w end
local function rowbase(parent,t,h) local f=Instance.new("Frame") f.Size=UDim2.new(1,0,0,h or 24) f.BackgroundColor3=C.hover f.BackgroundTransparency=1 f.BorderSizePixel=0 f.Parent=parent corner(f,4) f.MouseEnter:Connect(function() TweenService:Create(f,TweenInfo.new(0.1),{BackgroundTransparency=0}):Play() end) f.MouseLeave:Connect(function() TweenService:Create(f,TweenInfo.new(0.1),{BackgroundTransparency=1}):Play() end) local lbl=Instance.new("TextLabel") lbl.Size=UDim2.new(1,-40,1,0) lbl.Position=UDim2.fromOffset(6,0) lbl.BackgroundTransparency=1 lbl.Font=FONTM lbl.Text=t lbl.TextColor3=C.text lbl.TextSize=10 lbl.TextXAlignment=Enum.TextXAlignment.Left lbl.TextTruncate=Enum.TextTruncate.AtEnd lbl.Parent=f return f,lbl end
local function toggle(parent,name,default,cb) local f=rowbase(parent,name,24) local sw=Instance.new("TextButton") sw.Size=UDim2.fromOffset(26,14) sw.Position=UDim2.new(1,-32,0.5,-7) sw.BackgroundColor3=default and C.blue or C.trackOff sw.Text="" sw.AutoButtonColor=false sw.Parent=f corner(sw,7) local knob=Instance.new("Frame") knob.Size=UDim2.fromOffset(10,10) knob.Position=default and UDim2.new(1,-12,0.5,-5) or UDim2.new(0,2,0.5,-5) knob.BackgroundColor3=C.knob knob.BorderSizePixel=0 knob.Parent=sw corner(knob,5) local st=default sw.MouseButton1Click:Connect(function() st=not st TweenService:Create(sw,TweenInfo.new(0.12),{BackgroundColor3=st and C.blue or C.trackOff}):Play() TweenService:Create(knob,TweenInfo.new(0.12,Enum.EasingStyle.Quad),{Position=st and UDim2.new(1,-12,0.5,-5) or UDim2.new(0,2,0.5,-5)}):Play() cb(st) end) end
local function slider(parent,name,min,max,default,cb) local f=rowbase(parent,"",32) local lbl=f:FindFirstChildOfClass("TextLabel") lbl.Size=UDim2.new(1,-12,0,13) lbl.Position=UDim2.fromOffset(6,2) lbl.TextColor3=C.dim lbl.Text=name local val=Instance.new("TextLabel") val.Size=UDim2.fromOffset(32,13) val.Position=UDim2.new(1,-38,0,2) val.BackgroundTransparency=1 val.Font=FONTB val.Text=tostring(default) val.TextColor3=C.text val.TextSize=10 val.TextXAlignment=Enum.TextXAlignment.Right val.Parent=f local bar=Instance.new("Frame") bar.Size=UDim2.new(1,-12,0,3) bar.Position=UDim2.fromOffset(6,21) bar.BackgroundColor3=C.trackOff bar.BorderSizePixel=0 bar.Parent=f corner(bar,1.5) local fill=Instance.new("Frame") fill.Size=UDim2.new((default-min)/(max-min),0,1,0) fill.BackgroundColor3=C.blue fill.BorderSizePixel=0 fill.Parent=bar corner(fill,1.5) local knob=Instance.new("Frame") knob.Size=UDim2.fromOffset(8,8) knob.AnchorPoint=Vector2.new(0.5,0.5) knob.Position=UDim2.new((default-min)/(max-min),0,0.5,0) knob.BackgroundColor3=C.knob knob.BorderSizePixel=0 knob.Parent=bar corner(knob,4) local drag=false local function setX(x) local rel=math.clamp((x-bar.AbsolutePosition.X)/bar.AbsoluteSize.X,0,1) local v=math.floor(min+(max-min)*rel+0.5) fill.Size=UDim2.new(rel,0,1,0) knob.Position=UDim2.new(rel,0,0.5,0) val.Text=tostring(v) cb(v) end bar.InputBegan:Connect(function(i) if i.UserInputType==Enum.UserInputType.MouseButton1 or i.UserInputType==Enum.UserInputType.Touch then drag=true setX(i.Position.X) end end) knob.InputBegan:Connect(function(i) if i.UserInputType==Enum.UserInputType.MouseButton1 or i.UserInputType==Enum.UserInputType.Touch then drag=true end end) conn(UIS.InputEnded,function(i) if i.UserInputType==Enum.UserInputType.MouseButton1 or i.UserInputType==Enum.UserInputType.Touch then drag=false end end) conn(UIS.InputChanged,function(i) if drag and (i.UserInputType==Enum.UserInputType.MouseMovement or i.UserInputType==Enum.UserInputType.Touch) then setX(i.Position.X) end end) end
local function pill(parent,name,iconName,cb) local b=Instance.new("TextButton") b.Size=UDim2.new(1,0,0,24) b.BackgroundColor3=C.pill b.Text="" b.AutoButtonColor=false b.Parent=parent corner(b,5) strokeOf(b,C.pillBrd,0) if iconName then local ic=lucide(b,iconName,10,C.dim) ic.Position=UDim2.new(0,8,0.5,-5) end local l=Instance.new("TextLabel") l.Size=UDim2.new(1,-30,1,0) l.Position=UDim2.fromOffset(iconName and 24 or 8,0) l.BackgroundTransparency=1 l.Font=FONTM l.Text=name l.TextColor3=C.text l.TextSize=9 l.TextXAlignment=Enum.TextXAlignment.Left l.Parent=b b.MouseEnter:Connect(function() TweenService:Create(b,TweenInfo.new(0.1),{BackgroundColor3=C.hover}):Play() end) b.MouseLeave:Connect(function() TweenService:Create(b,TweenInfo.new(0.1),{BackgroundColor3=C.pill}):Play() end) b.MouseButton1Click:Connect(cb) end

-- COLUMNS (4 أعمدة بنفس الحجم الصغير)
local c1=column(1)
section(c1,"Aimbot"..(HAS_MOUSEMOVE and "" or " ⚠"),"crosshair",true)
toggle(c1,"Aimbot",false,function(o) State.Aimbot=o end)
toggle(c1,"Hold RMB",true,function(o) State.AimHold=o end)
toggle(c1,"Head",true,function(o) State.AimPart=o and "Head" or "HumanoidRootPart" end)
toggle(c1,"Team",false,function(o) State.AimTeamCheck=o end)
slider(c1,"FOV",20,500,State.AimFov,function(v) State.AimFov=v end)
slider(c1,"Smooth",1,30,State.AimSmooth,function(v) State.AimSmooth=v end)
section(c1,"Camera","camera")
slider(c1,"FOV",30,120,70,function(v) Workspace.CurrentCamera.FieldOfView=v end)
...
toggle(c2,"Inf Jump",false,function(o) State.InfJump=o end)
slider(c2,"Fly Spd",10,300,State.FlySpeed,function(v) State.FlySpeed=v end)
slider(c2,"Walk Spd",16,500,State.WalkSpeed,function(v) State.WalkSpeed=v local h=getHum() if h then h.WalkSpeed=v end end)
slider(c2,"Jump Pwr",50,500,State.JumpPower,function(v) State.JumpPower=v local h=getHum() if h then h.UseJumpPower=true h.JumpPower=v end end)
...
toggle(c3,"ESP",false,function(o) State.ESP=o end)
toggle(c3,"Boxes",true,function(o) State.ESPBoxes=o end)
toggle(c3,"Names",true,function(o) State.ESPNames=o end)
toggle(c3,"Tracers",false,function(o) State.ESPTracers=o end)
toggle(c3,"Health",true,function(o) State.ESPHealth=o end)
toggle(c3,"Team Chk",false,function(o) State.ESPTeam=o end)
section(c3,"Tools","wrench")
pill(c3,"IY","terminal",function() loadTool("Infinite Yield","https://raw.githubusercontent.com/EdgeIY/infiniteyield/master/source") end)
pill(c3,"Dex","folder-tree",function() loadTool("Dex","https://raw.githubusercontent.com/infyiff/backup/main/dex.lua") end)
pill(c3,"Spy","radar",function() loadTool("SimpleSpy","https://raw.githubusercontent.com/78n/SimpleSpy/main/SimpleSpySource.lua") end)

local c4=column(4)
section(c4,"Players","users",true)
local plistWrap=Instance.new("Frame") plistWrap.Size=UDim2.new(1,0,0,130) plistWrap.BackgroundColor3=C.bg plistWrap.BackgroundTransparency=0.4 plistWrap.BorderSizePixel=0 plistWrap.Parent=c4 corner(plistWrap,5)
local plist=Instance.new("ScrollingFrame") plist.Size=UDim2.new(1,-2,1,-2) plist.Position=UDim2.fromOffset(1,1) plist.BackgroundTransparency=1 plist.BorderSizePixel=0 plist.ScrollBarThickness=2 plist.ScrollBarImageColor3=C.border plist.CanvasSize=UDim2.new() plist.AutomaticCanvasSize=Enum.AutomaticSize.Y plist.Parent=plistWrap
local pl=Instance.new("UIListLayout") pl.Padding=UDim.new(0,2) pl.SortOrder=Enum.SortOrder.Name pl.Parent=plist
local pp=Instance.new("UIPadding") pp.PaddingTop=UDim.new(0,1) pp.PaddingLeft=UDim.new(0,1) pp.PaddingRight=UDim.new(0,1) pp.Parent=plist
local function miniBtn(parent,txt,xoff,cb,col) local b=Instance.new("TextButton") b.Size=UDim2.fromOffset(22,16) b.Position=UDim2.new(1,xoff,0.5,-8) b.BackgroundColor3=col or C.pill b.Text=txt b.Font=FONTM b.TextSize=8 b.TextColor3=C.text b.AutoButtonColor=true b.Parent=parent corner(b,4) strokeOf(b,C.pillBrd,0.3) b.MouseButton1Click:Connect(cb) return b end
local function refreshPlayers() for _,ch in ipairs(plist:GetChildren()) do if ch:IsA("Frame") then ch:Destroy() end end for _,plr in ipairs(Players:GetPlayers()) do if plr~=LP then local f=Instance.new("Frame") f.Name=plr.Name f.Size=UDim2.new(1,0,0,20) f.BackgroundColor3=C.hover f.BackgroundTransparency=1 f.BorderSizePixel=0 f.Parent=plist corner(f,4) f.MouseEnter:Connect(function() f.BackgroundTransparency=0 end) f.MouseLeave:Connect(function() f.BackgroundTransparency=1 end) local n=Instance.new("TextLabel") n.Size=UDim2.new(1,-48,1,0) n.Position=UDim2.fromOffset(5,0) n.BackgroundTransparency=1 n.Font=FONTM n.Text=plr.DisplayName n.TextColor3=C.text n.TextSize=9 n.TextXAlignment=Enum.TextXAlignment.Left n.TextTruncate=Enum.TextTruncate.AtEnd n.Parent=f miniBtn(f,"TP",-26,function() tpTo(plr) end) miniBtn(f,"FL",-2,function() flingPlayer(plr) end,C.red) end end end
refreshPlayers()
conn(Players.PlayerAdded,function() task.wait(0.2) refreshPlayers() end)
conn(Players.PlayerRemoving,function() task.wait(0.1) refreshPlayers() end)
section(c4,"Utility","settings")
pill(c4,"Spawn","flag",function() local root=getRoot() local sp=Workspace:FindFirstChildOfClass("SpawnLocation") if root and sp then root.CFrame=sp.CFrame+Vector3.new(0,5,0) end end)
pill(c4,"Reset","rotate-ccw",function() local h=getHum() if h then h.Health=0 end end)
pill(c4,"Rejoin","refresh-cw",function() TeleportService:Teleport(game.PlaceId,LP) end)

-- Equalize heights + scaling
local cols={c1,c2,c3,c4}
task.spawn(function() for _=1,4 do RunService.Heartbeat:Wait() end local maxH=0 for _,col in ipairs(cols) do maxH=math.max(maxH,col.AbsoluteSize.Y) end for _,col in ipairs(cols) do col.AutomaticSize=Enum.AutomaticSize.None col.Size=UDim2.fromOffset(COLW,maxH) end main.AutomaticSize=Enum.AutomaticSize.None body.AutomaticSize=Enum.AutomaticSize.None body.Size=UDim2.new(1,0,0,maxH+12) main.Size=UDim2.fromOffset(WIN_W,34+maxH+12) end)

-- زر تصغير/تكبير (تكبير/تصغير اللوحة بنسبة 0.6x - 1.4x)
local scaleState=1
local function applyScale(s)
    s=math.clamp(s,0.6,1.4)
    scaleState=s
    main.Size=UDim2.new(0,WIN_W*s,0,(34+body.AbsoluteSize.Y+12)*s)
    main.Position=UDim2.new(0.5,-WIN_W*s/2,0,40)
end
sizeBtn.MouseButton1Click:Connect(function()
    local newScale=scaleState==1 and 0.7 or scaleState==0.7 and 1.4 or 1
    applyScale(newScale)
end)

local function cleanup() stopFly() State.Noclip=false State.Aimbot=false State.ESP=false stopFling() for plr,_ in pairs(_G.OlemadESP) do removeESP(plr) end end
...
conn(UIS.InputBegan,function(i,gp) if gp then return end if i.KeyCode==Enum.KeyCode.RightShift then main.Visible=not main.Visible end end)
print("[Olemad] v5.1 Mobile loaded | Scale: "..scaleState)
